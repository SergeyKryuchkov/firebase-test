...
const { auth, firestore } = require('../../helpers/firebase');
...
router.post('/signup',
    ...
    errors.wrap(async (req, res) => {
        ...
        try {
            ...
            const customToken = await auth.createCustomToken(user.uuid);
            consooe.log(customToken);
            // const user123 = await auth.createUserWithEmailAndPassword(user.email, user.password);
            // console.log(user123);
            // user123.credits = 1000000;
            // await user123.save();
            // const fbUser = await firestore.collection('users').add({
            //     email: user.email,
            //     credits: 100000,
            // });
            // console.log(fbUser);
